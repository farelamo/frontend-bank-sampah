    ...
    <div class="bg-white rounded-xl p-6">
      <div class="flex justify-between mb-6">
        ...
        <div class="flex justify-content-end">
          <router-link v-if="!isNasabah" to="/trash/add">
            <button type="button"
              class="text-white bg-primary hover:bg-green-800 focus:ring-4 focus:ring-blue-300 font-light rounded-lg text-sm py-2 px-4 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">
              <span class="pr-2">+</span>Tambah Sampah
            </button>
          </router-link>
          <router-link to="#">
            <button v-if="!isNasabah" @click="toggleExcelModal" type="button"
      ...
      <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
          <thead class="text-xs text-gray-700 uppercase bg-gray-100 dark:bg-gray-700 dark:text-gray-400">
            <tr>
              ...
              <th v-if="!isNasabah" scope="col" class="px-3 py-3">
                Aksi
              </th>
            </tr>
          </thead>
            ...
            <tr v-for="(trash, index ) in trashes" :key="trash.id"
                ...
                <div class="flex gap-1">
                  <div @click="editItem(trash.id)" class="p-1 rounded bg-black cursor-pointer">
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                      ...
                      <path d="M5.96875 13.4688L2.53125 10.0312" stroke="white" stroke-linecap="round"
              ...
              <Modal @close="toggleModal" :modalActive="modalActive">
                ...
                <div class="flex justify-center pt-2">
                  <button @click="close"
                    class="px-4 bg-transparent p-3 rounded-lg text-red-600 hover:bg-gray-100 hover:text-red-400 mr-2">Cancel</button>
                  <button @click="deleteItem(trash.id)"
                    class="modal-close px-4 bg-red-600 p-3 rounded-lg text-white hover:bg-red-400">Hapus</button>
                </div>
              </Modal>
            </tr>
          </tbody>
        ...
        <ModalDefault @close="toggleExcelModal" :modalDefaultActive="modalExcelActive">
          <div class="modal-content">
            <h1 class="text-center">Upload Data Sampah (Excel)</h1>
      ...
      <div class="flex flex-col items-center mt-4">
        ...
        <span class="text-sm text-gray-700 dark:text-gray-400">
          Showing <span class="font-semibold text-gray-900 dark:text-white">{{ currentPage }}</span> to <span
            class="font-semibold text-gray-900 dark:text-white">{{ totalPages }}</span> of <span
